...
대표적인 예로는 **인터루킨-2**(IL-2) 관련 결함 마우스에서 관찰되는 심각한 염증과 자가면역 양상, **CTLA-4** 결손 마우스에서 나타나는 광범위한 T 세포 활성화와 다장기 염증 및 조기 사망, 그리고 **scurfy** 마우스에서 나타나는 전신 염증과 통제되지 않는 CD4⁺ T 세포 반응이 있었습니다.
...
2001년 전후로 FOXP3가 주목받기 시작한 이유는 두 갈래의 증거가 거의 동시에 쌓였기 때문입니다. 하나는 scurfy 마우스에서 문제가 되는 유전적 결함이 FOXP3와 연결되었다는 점이고, 다른 하나는 사람의 **IPEX 증후군**(Immune dysregulation, Polyendocrinopathy, Enteropathy, X-linked) 환자에서도 FOXP3 변이가 확인되었다는 점입니다.
