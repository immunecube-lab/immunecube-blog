...
제조 과정은 공여자 B세포를 EBV로 감염시켜 림프모구세포주(LCL)를 만든 뒤, 이를 항원제시세포로 반복 자극하고, 고농도 **인터루킨-2**(interleukin-2, IL-2)를 공급해 EBV 특이 T세포만 선택적으로 확장하는 방식이었습니다. 이렇게 얻은 CTL은 이미 EBV DNA가 상승한 초기 단계 환자에게는 선제적 치료로, 고위험군 환자에게는 예방 목적으로 투여되었습니다. 이는 단순한 세포 투여가 아니라, **모니터링 기반 면역 개입 모델**의 시작이었습니다.
