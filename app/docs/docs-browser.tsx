...
import { getDocHref, groupByCategory, groupBySection, pickDisplayDate, resolveSelection, sectionLabel, type DocBrowserItem, type DocsSelection } from "./docs-browser-model";

export function DocsBrowser({ docs }: { docs: DocBrowserItem[] }) {
  const grouped = groupByCategory(docs);
  const categories = grouped.map(([category]) => category);
  const defaultCategory = categories[0] ?? "";
  const [selection, setSelection] = useState<DocsSelection>({
  ...
  function renderDocItem(doc: DocBrowserItem, featured = false) {
